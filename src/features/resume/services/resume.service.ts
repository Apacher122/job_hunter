...
export const compile_resume = async (id: number): Promise<void> => {  
  ...
  if (!jobContent || !jobContent.companyName) {
    console.error('Could not compile: Job posting content or company name is not available in infoStore.');
    return;
  }
  try {
    ...
    await exportLatex({
      ...
      compiledPdfPath: paths.paths.compiledResume(jobContent.companyName, jobContent.id),
      movedPdfPath: paths.paths.movedResume(jobContent.companyName, jobContent.id),
      jobPost: jobContent 
    });
...
const generateFullResume = async (content: JobPosting): Promise<void> => {
  console.log(`RESUME - creating resume for ${content.rawCompanyName} - ${content.position}`)
  try {
    ...
    const instructions = await loadTemplate('instructions', 'resume', { mistakes: mistakesMade, });
    ...
    if (!res) throw new Error("Failed to get response from OpenAI for full resume generation");
    ...
    await fs.promises.writeFile(paths.paths.jsonResume(content.companyName, content.id), JSON.stringify(parsedResponse.data, null, 2));
    ...
    console.error(`Error generating full resume for ${content.companyName}:`, err);
    throw err;
  }
}
