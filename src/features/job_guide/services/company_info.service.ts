...
export const getCompanyInfo = async (id: number): Promise<any> => {
  ...
  const resumeData = await fs.promises.readFile(
    paths.paths.jsonResume(jobPost.companyName, jobPost.id)
  );
  const instructions = await loadTemplate('instructions', 'companyinfo', {
    company: jobPost.companyName,
    position: jobPost.position,
  });

  const prompt = await loadTemplate('prompts', 'companyinfo', {
    resume: resumeData,
  ...
  await upsertCompanyInfo(jobPost.id, response);
};
