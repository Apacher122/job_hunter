...
import { insertRowToSheet } from '../shared/apis/google/sheets';

const filePathFor = (companyName: string, suffix: 'resume' | 'cover-letter') =>
    suffix === 'resume'
? paths.paths.moved_resume(companyName)
: paths.paths.moved_cover_letter(companyName);
...
export const downloadDocument = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        res.status(500).send(`Failed to handle ${docType}`);
    }
};
